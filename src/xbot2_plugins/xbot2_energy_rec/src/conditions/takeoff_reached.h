...
namespace BT
{
    ...
    class TakeoffReached : public ConditionNode, public Task
    {
        public:

            // You must provide the function to call when tick() is invoked
            TakeoffReached(const std::string& name, const NodeConfiguration& config);

            ~TakeoffReached() override = default;
