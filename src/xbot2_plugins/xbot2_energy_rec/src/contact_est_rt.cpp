...
{
    _fest = std::make_shared<XBot::Cartesian::Utils::ForceEstimationMomentumBased>(
            _base_est_model,
            1.0/_plugin_dt, // rate @ which the obs. dynamics is integrated
            _svd_thresh, // regularization for SVD computation
            _ft_est_bw); // observer bandwidth);

    _fest_virt = _fest->add_link(_contact_linkname, _selector);
...
{    
    ...
    _pin_model_ptr->set_q(_q_p_ft_est);
    _pin_model_ptr->set_v(_q_p_dot_ft_est);
    _pin_model_ptr->set_a(_q_p_ddot_ft_est);
    _pin_model_ptr->set_tau(_tau_ft_est);
...
//    _base_est_model->getOrientation(_tip_link_name, _R_world_from_tip);
    ...
    _pin_model_ptr->get_v(_v);
    _pin_model_ptr->get_g(_g);
    _pin_model_ptr->get_p(_p);
    _pin_model_ptr->get_tau(_tau);
