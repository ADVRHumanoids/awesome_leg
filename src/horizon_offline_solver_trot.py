...
if  (not scibidibi.path.isdir(rospackage.get_path("awesome_leg_pholus")+"/media/horizon_trot/"+today_is)):
    scibidibi.makedirs(rospackage.get_path("awesome_leg_pholus")+"/media/horizon_trot/"+today_is)

if  (not scibidibi.path.isdir(rospackage.get_path("awesome_leg_pholus")+"/media/horizon_trot/"+today_is)):
    scibidibi.mkdir(rospackage.get_path("awesome_leg_pholus")+"/media/horizon_trot/"+today_is+"/first_version")

##

ms = mat_storer.matStorer(rospackage.get_path("awesome_leg_pholus")+"/opt_results/horizon_trot/first_version/horizon_offline_solver.mat")
target=rospackage.get_path("awesome_leg_pholus")+"/media/horizon_trot/"+today_is+"/first_version/"
ms_aux = mat_storer.matStorer(target+"horizon_offline_solver.mat")

shutil.copyfile(config_path+"actuators.yaml", target+"actuators.yaml") # saving config files for reference and future debugging
shutil.copyfile(config_path+"horizon_gazebo_cosim.yaml", target+"horizon_gazebo_cosim.yaml")
shutil.copyfile(config_path+"horizon_solver_trot.yaml", target+"horizon_solver.yaml")
...
if save_sol_as_init: # save the solution as the initialization for the next sim
    ms_opt_init = mat_storer.matStorer(rospackage.get_path("awesome_leg_pholus")+"/opt_results/horizon_trot/first_version/horizon_offline_solver_init.mat")
if employ_opt_init: # initialize variables with the previously saved solution
    ms_load_path=rospackage.get_path("awesome_leg_pholus")+"/opt_results/horizon_trot/first_version/horizon_offline_solver_init.mat"
...
# weight_hip_i_d=rospy.get_param("/horizon_solver/variable_dt/problem_settings/cost_weights/weight_hip_i_d") # minimizing hip i_d
# weight_knee_i_d=rospy.get_param("/horizon_solver/variable_dt/problem_settings/cost_weights/weight_knee_i_d") # minimizing hip i_d

# solver options

slvr_opt = {"ipopt.tol": rospy.get_param("/horizon_solver/variable_dt/problem_settings/solver/tolerance"), 
"ipopt.max_iter": rospy.get_param("/horizon_solver/variable_dt/problem_settings/solver/max_iter"), 
"ipopt.linear_solver": rospy.get_param("/horizon_solver/variable_dt/problem_settings/solver/linear_solver_name")} 
