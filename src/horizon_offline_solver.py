...
if os.path.exists(rospackage.get_path("awesome_leg_pholus")+"/sim_results/horizon_offline_solver.mat"):
    os.remove(rospackage.get_path("awesome_leg_pholus")+"/sim_results/horizon_offline_solver.mat")
...
if is_adaptive_dt:
    ...
    if is_single_dt:
        ...
        n_nodes = rospy.get_param("/horizon_solver/variable_dt/single_dt/problem_settings/n_nodes")  # optimization horizon
        n_takeoff =rospy.get_param("/horizon_solver/variable_dt/single_dt/problem_settings/n_takeoff")   # instant of takeoff
        n_touchdown =rospy.get_param("/horizon_solver/variable_dt/single_dt/problem_settings/n_touchdown")   # instant of touchdown

        dt_lb=rospy.get_param("/horizon_solver/variable_dt/single_dt/problem_settings/dt_lb")   # dt lower bound 
        dt_ub=rospy.get_param("/horizon_solver/variable_dt/single_dt/problem_settings/dt_ub")   # dt upper bound
        ...
        test_rig_ub=rospy.get_param("/horizon_solver/variable_dt/single_dt/problem_settings/test_rig/ub") # upper bound of the test rig excursion
        # hip_lb= rospy.get_param("/horizon_solver/variable_dt/single_dt/problem_settings/hip/lb") # lower bound of the hip joint
        # hip_ub=rospy.get_param("/horizon_solver/variable_dt/single_dt/problem_settings/hip/ub") # upper bound of the hip joint
        # top_lb= rospy.get_param("/horizon_solver/variable_dt/single_dt/problem_settings/knee/lb") # lower bound of the hip joint
        # top_ub=rospy.get_param("/horizon_solver/variable_dt/single_dt/problem_settings/knee/ub") # upper bound of the hip joint
        q_p_init = rospy.get_param("/horizon_solver/variable_dt/single_dt/problem_settings/initial_conditions/q_p") # initial joint config (ideally it would be given from measurements)
        q_p_dot_init = rospy.get_param("/horizon_solver/variable_dt/single_dt/problem_settings/initial_conditions/q_p_dot") # initial joint config (ideally it would be given from measurements)

        # cost weights

        weight_contact_cost = rospy.get_param("/horizon_solver/variable_dt/single_dt/problem_settings/cost_weights/contact_force")  # minimizing the contact force
        # weight_postural_cost = rospy.get_param("/horizon_solver/variable_dt/single_dt/problem_settings/cost_weights/terminal_postural") # cost to restore the initial position at the end of the control horizon
        weight_q_ddot = rospy.get_param("/horizon_solver/variable_dt/single_dt/problem_settings/cost_weights/small_q_p_ddot")# minimizing joint accelerations
        weight_hip_height_jump = rospy.get_param("/horizon_solver/variable_dt/single_dt/problem_settings/cost_weights/big_hip_jump") # maximizing the jump height (measured at the hip)
        weight_tip_clearance=rospy.get_param("/horizon_solver/variable_dt/single_dt/problem_settings/cost_weights/big_foot_tip_clearance") # maximizing the jump height (measured at the tip)
        weight_hip_i_d=rospy.get_param("/horizon_solver/variable_dt/single_dt/problem_settings/cost_weights/weight_hip_i_d") # minimizing hip i_d
        weight_knee_i_d=rospy.get_param("/horizon_solver/variable_dt/single_dt/problem_settings/cost_weights/weight_knee_i_d") # minimizing hip i_d
        
        # solver options

        slvr_opt = {"ipopt.tol": rospy.get_param("/horizon_solver/variable_dt/single_dt/problem_settings/solver/tolerance"), 
        "ipopt.max_iter": rospy.get_param("/horizon_solver/variable_dt/single_dt/problem_settings/solver/max_iter"), 
        "ipopt.linear_solver": rospy.get_param("/horizon_solver/variable_dt/single_dt/problem_settings/solver/linear_solver_name")} 
        slvr_name=rospy.get_param("/horizon_solver/variable_dt/single_dt/problem_settings/solver/name") # options: "blocksqp", "ipopt", "ilqr", "gnsqp", 

    
    else:

        n_nodes = rospy.get_param("/horizon_solver/variable_dt/multiple_dt/problem_settings/n_nodes")  # optimization horizon
        n_takeoff =rospy.get_param("/horizon_solver/variable_dt/multiple_dt/problem_settings/n_takeoff")   # instant of takeoff
        n_touchdown =rospy.get_param("/horizon_solver/variable_dt/multiple_dt/problem_settings/n_touchdown")   # instant of touchdown

        dt_lb=rospy.get_param("/horizon_solver/variable_dt/multiple_dt/problem_settings/dt_lb")   # dt lower bound 
        dt_ub=rospy.get_param("/horizon_solver/variable_dt/multiple_dt/problem_settings/dt_ub")   # dt upper bound
        test_rig_lb= rospy.get_param("horizon_solver/variable_dt/multiple_dt/problem_settings/test_rig/lb") # lower bound of the test rig excursion
        test_rig_ub=rospy.get_param("/horizon_solver/variable_dt/multiple_dt/problem_settings/test_rig/ub") # upper bound of the test rig excursion
        # hip_lb= rospy.get_param("/horizon_solver/variable_dt/multiple_dt/problem_settings/hip/lb") # lower bound of the hip joint
        # hip_ub=rospy.get_param("/horizon_solver/variable_dt/multiple_dt/problem_settings/hip/ub") # upper bound of the hip joint
        # top_lb= rospy.get_param("/horizon_solver/variable_dt/multiple_dt/problem_settings/knee/lb") # lower bound of the hip joint
        # top_ub=rospy.get_param("/horizon_solver/variable_dt/multiple_dt/problem_settings/knee/ub") # upper bound of the hip joint
        q_p_init = rospy.get_param("/horizon_solver/variable_dt/multiple_dt/problem_settings/initial_conditions/q_p") # initial joint config (ideally it would be given from measurements)
        q_p_dot_init = rospy.get_param("/horizon_solver/variable_dt/multiple_dt/problem_settings/initial_conditions/q_p_dot") # initial joint config (ideally it would be given from measurements)

        # cost weights

        weight_contact_cost = rospy.get_param("/horizon_solver/variable_dt/multiple_dt/problem_settings/cost_weights/contact_force")  # minimizing the contact force
        # weight_postural_cost = rospy.get_param("/horizon_solver/variable_dt/multiple_dt/problem_settings/cost_weights/terminal_postural") # cost to restore the initial position at the end of the control horizon
        weight_q_ddot = rospy.get_param("/horizon_solver/variable_dt/multiple_dt/problem_settings/cost_weights/small_q_p_ddot")# minimizing joint accelerations
        weight_hip_height_jump = rospy.get_param("/horizon_solver/variable_dt/multiple_dt/problem_settings/cost_weights/big_hip_jump") # maximizing the jump height (measured at the hip)
        weight_tip_clearance=rospy.get_param("/horizon_solver/variable_dt/multiple_dt/problem_settings/cost_weights/big_foot_tip_clearance") # maximizing the jump height (measured at the tip)
        weight_hip_i_d=rospy.get_param("/horizon_solver/variable_dt/multiple_dt/problem_settings/cost_weights/weight_hip_i_d") # minimizing hip i_d
        weight_knee_i_d=rospy.get_param("/horizon_solver/variable_dt/multiple_dt/problem_settings/cost_weights/weight_knee_i_d") # minimizing hip i_d
        
        # solver options

        slvr_opt = {"ipopt.tol": rospy.get_param("/horizon_solver/variable_dt/multiple_dt/problem_settings/solver/tolerance"),
         "ipopt.max_iter": rospy.get_param("/horizon_solver/variable_dt/multiple_dt/problem_settings/solver/max_iter"), 
         "ipopt.linear_solver": rospy.get_param("/horizon_solver/variable_dt/multiple_dt/problem_settings/solver/linear_solver_name")} 
        slvr_name=rospy.get_param("/horizon_solver/variable_dt/multiple_dt/problem_settings/solver/name") # options: "blocksqp", "ipopt", "ilqr", "gnsqp", 
    ...
    weight_hip_i_d=rospy.get_param("/horizon_solver/constant_dt/problem_settings/cost_weights/weight_hip_i_d") # minimizing hip i_d
    weight_knee_i_d=rospy.get_param("/horizon_solver/constant_dt/problem_settings/cost_weights/weight_knee_i_d") # minimizing hip i_d
    
    # solver options

    slvr_opt = {"ipopt.tol": rospy.get_param("/horizon_solver/constant_dt/problem_settings/solver/tolerance"),
     "ipopt.max_iter": rospy.get_param("/horizon_solver/constant_dt/problem_settings/solver/max_iter"),
      "ipopt.linear_solver": rospy.get_param("/horizon_solver/constant_dt/problem_settings/solver/linear_solver_name")}
...
useful_solutions={"q_p":solution["q_p"][1:3,:],"q_p_dot":solution["q_p_dot"][1:3,:], "q_p_ddot":solution["q_p_ddot"][1:3,:], "tau":cnstr_opt["tau_limits"], "f_contact":solution["f_contact"], "i_q":i_q, "dt_opt":slvr.getDt(), "foot_tip_height":np.transpose(solution_foot_tip_position-init_solution_foot_tip_position_aux[2,:]), "hip_height":np.transpose(solution_hip_position-init_solution_hip_position_aux[2,:]), "sol_time":solution_time}
