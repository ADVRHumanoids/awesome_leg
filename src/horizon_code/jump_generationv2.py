...
urdf_kin_dyn = casadi_kin_dyn.py3casadi_kin_dyn.CasadiKinDyn(urdf)

n_q = urdf_kin_dyn.nq()  # number of joints
n_v = urdf_kin_dyn.nv()  # number of dofs
...
lbs = urdf_kin_dyn.q_min() + jnt_lim_margin_array
ubs = urdf_kin_dyn.q_max() - jnt_lim_margin_array
...
tau = prb.createInputVariable("tau", n_q)  # using joint accelerations as an input variable
if is_refine_phase:
    tau.setInitialGuess(loaded_sol["tau_res"])
...
forw_dyn = kin_dyn.ForwardDynamics(urdf_kin_dyn, contact_map.keys(),\
    casadi_kin_dyn.py3casadi_kin_dyn.CasadiKinDyn.LOCAL_WORLD_ALIGNED)
q_p_ddot = forw_dyn.call(q_p, q_p_dot, tau, contact_map)
...
# hip
fk_hip = cs.Function.deserialize(urdf_kin_dyn.fk(shank_name))  # deserializing
...
dfk_hip = cs.Function.deserialize(
    urdf_kin_dyn.frameVelocity(shank_name,\
            casadi_kin_dyn.py3casadi_kin_dyn.CasadiKinDyn.LOCAL_WORLD_ALIGNED))
v_hip = dfk_hip(q = q_p, qdot = q_p_dot)["ee_vel_linear"]  # foot velocity

# foot tip pos
fk_foot = cs.Function.deserialize(urdf_kin_dyn.fk("tip"))
...
dfk_foot = cs.Function.deserialize(
    urdf_kin_dyn.frameVelocity("tip",\
        casadi_kin_dyn.py3casadi_kin_dyn.CasadiKinDyn.LOCAL_WORLD_ALIGNED))
v_foot_tip = dfk_foot(q=q_p, qdot=q_p_dot)["ee_vel_linear"]  # foot velocity

# center of mass
com_f = cs.Function.deserialize(urdf_kin_dyn.centerOfMass())
...
if is_refine_phase: # cost functions for the refinement phase
    ...
    weight_tau = rospy.get_param("horizon/horizon_solver/problem_settings/cost_weights/traj_refiner/weight_tau")# minimizing joint accelerations
    ...
    weight_tau = rospy.get_param("horizon/horizon_solver/problem_settings/cost_weights/weight_tau")# minimizing joint accelerations
...
weight_tau = weight_tau / cost_scaling_factor
prb.createIntermediateCost("min_tau", weight_tau * cs.sumsqr(
    tau[1:]))  # minimizing the joint accelerations ("responsiveness" of the trajectory)
...
prb.createIntermediateCost("min_input_diff", weight_min_input_diff * cs.sumsqr(tau[1:] - tau[1:].getVarOffset(-1)), 
                            nodes = range(1, n_int))  
...
dt_res = None
x_res = None
sol_contact_map = None
sol_contact_map = dict(tip = solution["f_contact"])

if resample_traj and not is_refine_phase:

    dae = {'x': x, 'p': cs.vertcat(tau, f_contact), 'ode': xdot, 'quad': 1}

    dt_res = rospy.get_param("horizon/horizon_resampler/dt")
    
    x_res = resampler_trajectory.resampler(solution["x_opt"],
                                            solution["u_opt"],
                                            slvr.getDt().flatten(),
                                            dt_res,
                                            dae) # contact forces are assumed constant over each sample interval (not true in reality)

## post-processing original solution

q_p_ddot_sol = forw_dyn.call(solution["q_p"][:, 1:], solution["q_p_dot"][:, 1:], solution["tau"], sol_contact_map, 
                            eval_num = True).toarray()

solution["q_p_ddot"] = q_p_ddot_sol 

# Hip and knee quadrature current estimation
i_q_n_samples = len(solution["tau"][0, :])
i_q = np.zeros((n_q - 1, i_q_n_samples))
for i in range(n_q - 1):
    
    for j in range(i_q_n_samples):

        compensated_tau = tau_sol[i + 1, j] + K_d0[i] * np.tanh( tanh_coeff * solution["q_p_dot"][i + 1, j]) + \
                                            K_d1[i] * solution["q_p_dot"][i + 1, j]
        
        i_q[i, j] = (rotor_axial_MoI[i] * solution["q_p_ddot"][i + 1, j] / red_ratio[i] +\
...
if not is_refine_phase:
    ...
    other_stuff = {"i_q":i_q, "dt_opt":slvr.getDt(),
...
res_GRF = None
res_sol_contact_map = None
if resample_traj and not is_refine_phase:

    ## post-processing original solution
    tau_res = resampler_trajectory.resample_input(solution["tau"],
                                        slvr.getDt().flatten(),
                                        dt_res)
    res_GRF = resampler_trajectory.resample_input(solution["f_contact"],
                                        slvr.getDt().flatten(),
                                        dt_res)
    p_res = x_res[0:n_q, :]
    v_res = x_res[n_q: n_q + n_v, :]
    res_sol_contact_map = dict(tip = res_GRF)
    q_p_ddot_res = forw_dyn.call(p_res[:, 1:], v_res[:, 1:], tau_res, res_sol_contact_map, eval_num = True).toarray()
    # Hip and knee quadrature current estimation
    n_res_samples = x_res[0: n_q, :].shape[1]
    i_q_res = np.zeros((n_q - 1, n_res_samples - 1))

    for i in range(n_q - 1):

        for j in range(n_res_samples - 1):

            compensated_tau = tau_res[i + 1, j] + K_d0[i] * np.tanh( tanh_coeff * v_res[i + 1, j]) +\
                                                K_d1[i] * v_res[i + 1, j]
            i_q_res[i, j] = (rotor_axial_MoI[i] * q_p_ddot_res[i + 1, j] / red_ratio[i] + \
                            compensated_tau * red_ratio[i] / efficiency[i]) / K_t[i]
    ...
    useful_solutions_res={"q_p_res":p_res,"q_p_dot_res":v_res, "q_p_ddot_res":q_p_ddot_res,
...
if replay_traj:
    joint_names = urdf_kin_dyn.joint_names()
    joint_names.remove("universe")  # removing the "universe joint"

    if resample_traj and not is_refine_phase:

        rpl_traj = replay_trajectory(dt_res, joint_names, x_res[0:n_q, :], res_sol_contact_map, \
                    cas_kin_dyn.CasadiKinDyn.LOCAL_WORLD_ALIGNED, urdf_kin_dyn)  # replaying the (resampled) trajectory
