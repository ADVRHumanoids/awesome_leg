...
if  (not scibidibi.path.isdir(media_path+"/"+today_is+"/adaptive_dt/")):
    scibidibi.mkdir(media_path+"/"+today_is+"/adaptive_dt")

## Various operations based on the selected options
        
ms = mat_storer.matStorer(opt_res_path + "/adaptive_dt/horizon_offline_solver.mat")  
ms_aux = mat_storer.matStorer(media_path + "/" + today_is + "/" + "/adaptive_dt/" + "horizon_offline_solver" + current_time + ".mat") 
target = media_path + "/" + today_is + "/adaptive_dt/"
...
shutil.copyfile(config_path + "horizon_jumpv2.yaml", target + "horizon" + current_time + ".yaml")

if save_sol_as_init: # save the solution as the initialization for the next sim
    ms_opt_init = mat_storer.matStorer(opt_res_path + "/adaptive_dt/" + opt_init_name + ".mat")
...
q_p[0].setBounds(test_rig_lb, test_rig_ub) # test rig excursion limits
